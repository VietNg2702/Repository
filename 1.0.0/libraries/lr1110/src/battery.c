...
void battery_read(uint16_t * val)
{
    ...
    VREF->AVCR = 0x00040011;    // VREF->AVCR_b.IBIASEN = 1U;
                                // VREF->AVCR_b.VREFEN  = 1U;
                                // VREF->AVCR_b.AVSEL   = 0U;
                                // VREF->AVCR_b.LPMD    = 0U;
                                // VREF->AVCR_b.STDMD   = 1U;

    /* P003 Set the GPIO to low to enable the battery voltage measurement circuitry. */
//    PORT0->PODR_b.PODR03 = 0;

//    DelayMs( 50 );
    system_time_wait_ms( 50 );
__NOP();
    /* Initialize S14AD */
    ...
    /* select VREFH0 */
//    adcDev->Control(AD_CMD_USE_VREFH0, NULL);
//    adcDev->Control(AD_CMD_USE_VREFH0, NULL);
...
//    /* P003 Set the GPIO to high to disable the battery voltage measurement circuitry. */
//    PORT0->PODR_b.PODR03 = 1;

    /* Disable S14AD */
    adcDev->Close();

    /* Disable VREF */
    VREF->AVCR = 0x00040000;    // VREF->AVCR_b.IBIASEN = 0U;
                                // VREF->AVCR_b.VREFEN  = 0U;
                                // VREF->AVCR_b.AVSEL   = 0U;
                                // VREF->AVCR_b.LPMD    = 0U;
                                // VREF->AVCR_b.STDMD   = 1U;
// VREF Modlue Stop

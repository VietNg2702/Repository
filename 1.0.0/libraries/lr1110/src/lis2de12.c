...
/**************************************************************************//**
* @brief  Sensor data read from IIS2DLPC Accelerometer
*
* @retval       
******************************************************************************/
...
//	ret1 = system_i2c_write(&Driver_I2C0, LIS2DE12_I2C_ADD,  (uint8_t*)lis2de12_buffer, 2 , true );
...
//    if (ret3 != SUCCESS) {return FAIL;}  // Read Fail
    bytes[0] = 0x0;
    ret3 = lis2de12_read_bytes(LIS2DE12_WHO_AM_I, &bytes[0], 1);
    APP_ERR_RETURN(ret3);
    if (bytes[0] == LIS2DE12_ID) {return 0;}
	else {return -1;}
}
...
int8_t lis2de12_data_fetch(accel_data_t * data)
{
    ...
    APP_ERR_RETURN(ret);
//    if (data16[0] >2047) {data16[0] -= 4096;}
//    if (data16[1] >2047) {data16[1] -= 4096;}
//    if (data16[2] >2047) {data16[2] -= 4096;}
    data->x = data16[0];
    data->y = data16[1];
    data->z = data16[2];
    return ret;
...
/**************************************************************************//**
* @brief  Sensor initialize to IIS2DLPC
*
* @retval       
******************************************************************************/
...
//    prepare_lis2de12_wakeup_function();
    return SUCCESS;  
}
